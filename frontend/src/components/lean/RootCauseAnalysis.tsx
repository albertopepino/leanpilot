    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-th-bg border border-th-border rounded-xl shadow-xl w-full max-w-lg p-6 space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-1 rounded hover:bg-th-bg-secondary transition-colors">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h2 className="text-xl font-bold text-th-text">Root Cause Analysis</h2>
        <button
          onClick={onNew}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-dashed border-th-border rounded-xl">
          <HelpCircle className="w-10 h-10 text-th-text-secondary mx-auto mb-3 opacity-40" />
          <p className="text-sm text-th-text-secondary">
            {search
              ? 'No analyses match your search.'
              : 'No root cause analyses yet. Start one to investigate problems systematically.'}
          </p>
        </div>
      ) : (
        /* Table */
        <div className="border border-th-border rounded-xl overflow-hidden">
          {/* Desktop header */}
          <div className="hidden sm:grid sm:grid-cols-[3rem_1fr_7rem_7rem_6rem_6rem] gap-2 px-4 py-2.5 bg-th-bg-secondary/60 text-xs font-semibold text-th-text-secondary uppercase tracking-wide">
            <span>ID</span>
          ...
          <div className="divide-y divide-th-border">
            {filtered.map((a) => (
              <button
                key={`${a.tool}-${a.id}`}
                onClick={() => onOpen(a)}
                className="w-full text-left sm:grid sm:grid-cols-[3rem_1fr_7rem_7rem_6rem_6rem] gap-2 px-4 py-3 hover:bg-th-bg-secondary/50 transition-colors flex flex-col sm:flex-row sm:items-center"
              >
                {/* ID */}
                <span className="text-xs text-th-text-secondary font-mono">#{a.id}</span>
                ...
                {/* Author */}
                <span className="text-xs text-th-text-secondary truncate">
                  {a.author || '-'}
                </span>
              </button>
            ))}
